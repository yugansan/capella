...
public class PropertyContext implements IPropertyContext {
  ...
  public void notifyListeners(IProperty property) {
    ...
    shouldNotify = false;
    if (properties != null) {
      for (IProperty prty : properties.getAllItems()) {
        if (prty instanceof ICompoundProperty) {
          ICompoundProperty cProperty = (ICompoundProperty) prty;
    ...
    for (IProperty prty : updated) {
      notifyListeners(prty);
    }
  }
